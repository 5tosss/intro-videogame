...
enemyImage.src = "/files/enemyOne.png";
...
window.onload = function () {
    if (confirm("¡Necesitamos acabar con los Tie-Fighters enemigos!,¿Entendido?")) {
...
function update(deltaTime) {
    ...
    for (let i = enemyProjectiles.length - 1; i >= 0; i--) {
        ...
        if (checkCollision(proj, player)) {
            ...
            if (player.damage >= 5) {
                ...
                alert("¡El imperio ha tomado la base rebelde!");
    ...
    // Condición de victoria: 30 enemigos destruidos
    if (enemyDestroyedCount >= 30) {
        stopTimer(); // Detener el cronómetro y guardar el mejor tiempo
        alert("Bien por ahora, le hemos dado el primer golpe al Imperio");
        window.location.href = "index.html";
        return;
    }
}
